...
def load_model():
    ...
    # activation model (for visualization)
    layer_outputs = [
        layer.output for layer in model.layers if isinstance(layer, Dense) or isinstance(layer, Flatten)
    ]
    activation_model = Model(inputs=model.input, outputs=layer_outputs)

    return model, activation_model

model, activation_model = load_model()
